...
/* ── 조각 렌더 ─────────────────────────────────────── */
/* ── 장비 색 ────────────────────────────────────────────
   현장은 늘 같은 장비 한둘을 본다. 이름을 읽기 전에 색으로 먼저 찾게 한다.
   면을 칠하지 않고 4px 띠·테두리에만 쓴다 — 내용 영역은 흰 바탕이다. */
var DEVCOLOR = {
  'XN': '#185adb', 'XN-L': '#0891b2', 'CN': '#4f46e5', 'SP-50': '#7c3aed',
  'UF': '#0d9488', 'UC': '#059669', 'HISCL': '#d97706', 'G11': '#e11d48',
  'TS-10': '#475569', 'CS-1600': '#0284c7', 'CA-620': '#c026d3'
};
function devColor(id) { return DEVCOLOR[id] || '#185adb'; }

/* 재발률 등급 — 현장이 정한 기준 (2026-08-25 · 실측 후 두 번 조정).
       30% 미만  낮음   ·  30~50%  보통  ·  50~60%  높음  ·  60% 이상  매우 높음
   판정 건수가 얇으면(10건 미만) 애초에 r 이 안 온다 — 그때는 'na'.

   왜 이 값인가 — 등급을 낼 수 있는 90종의 실제 분포로 정했다.
     최소 0 · 25% 24.2 · 중앙 35.9 · 75% 46.7 · 90% 61.1 · 최대 81.8
   중앙값 35.9 가 '보통' 한가운데 오도록 30/50 을 잡았다.
     낮음 37.8% · 보통 38.9% · 높음 12.2% · 매우 높음 11.1%

   지나온 자리
     20/40      목록의 44% 가 빨강이 되어 경고가 흔해졌다 → 버림
     30/50/70   '매우 높음' 이 5종(5.6%)뿐이라 너무 드물다는 현장 판단 → 60 으로 내림
   60% 는 '열 번 고치면 여섯 번 다시 부른다' 는 뜻이다.

   남은 문제 — 높음 11종 · 매우 높음 10종으로 두 칸의 크기가 같다.
   '매우' 가 '더 드물다' 는 뜻을 못 준다. 보통/높음 선을 50 → 45 로 내리면
   37.8 / 30.0 / 21.1 / 11.1 로 층이 고르게 줄어든다. 현장이 정할 일이라 남겨 둔다.

   주의 — 장비마다 판이 다르다 (중앙값 CN 52.8% ↔ UF 29.3%).
   하나의 잣대라 CN 은 빨강이 몰리고 UF 는 거의 없다. 등급은 전체 그림이 아니라
   **기록이 두꺼운 것들 안에서의 순위**로 읽어야 한다. */
var SEV_RULE = '30% 미만 낮음 · 30~50% 보통 · 50~60% 높음 · 60% 이상 매우 높음';
function sevOf(r) {
  if (r == null) return 'na';
  if (r >= 60) return 'top';
  if (r >= 50) return 'hi';
...
function rowHTML(r) {
  ...
  var sv = sevOf(r.r);
  var tags = '<span class="tag dv" style="--dc:' + devColor(r.d) + '">' + esc(r.d) + '</span>';
  ...
  return '<button class="row" data-go="' + r.d + '/' + r.p + '/' + r.i + '">' +
    '<span class="r1"><i class="sev ' + sv + '"></i>' +
    '<span class="t">' + esc(r.t) + '</span></span>' +
    '<span class="r2">' + tags + '</span>' +
    '<span class="r3"><span class="rec ' + sv + '">' + sevText(r.r) + '</span>' +
    '<span class="n">발생 ' + num(r.n) + '건</span></span>' +
    '</button>';
}
...
function renderHome() {
  ...
  // 장비 수·데이터 버전은 여기서 뺐다 (아래 오프라인 카드에 그대로 있다).
  // 그 자리에 문구를 올린다 — 앱을 열었을 때 첫 줄이 무엇인지가 인상을 정한다.
  subEl.className = 'slogan';
  subEl.innerHTML = '막혔을 때, 해결의 <b>실마리</b>를 찾다';
  backEl.hidden = true;
  homeEl.hidden = true;

  var devs = META.devices.map(function (d) {
    // '22 + 147' 은 무슨 뜻인지 화면에서 알 수 없었다. 이름을 붙여 준다.
...
function bar(cs) {
  ...
  return '<div class="share">' + cs.items.slice(0, 5).map(function (x) {
    ...
    '<p class="muted">조치 기록 ' + num(cs.base || cs.total) + '건에서 ' +
    '<b>맨 처음 한 조치</b> ' + num(cs.total) + '건만 세었습니다. ' +
    '무엇을 했는지로 <b>거꾸로 짚은 추정</b>입니다 — 원인을 확인한 결과가 아닙니다.' +
...
function crmHTML(e) {
  ...
  b += '<h4>현장 조치 (작업 기록 ' + num(c.base || 0) + '건 집계)</h4>' + stagesHTML(e.steps) +
    ...
    '<b>문제가 난 뒤에 이 조치를 하고 「양호」로 끝난</b> 것이 N건이라는 뜻입니다. ' +
...
function itemsHTML(e) {
  ...
  return '<details class="acc dat"><summary>항목별로 나눠 보기 (WBC·RBC…)<span class="src">데이터</span>' +
...
function partsHTML(e) {
    ...
    b += '<h4>많이 교체한 순 — 출동 전 챙길 부품</h4>' +
      ...
      pn.slice(0, 10).map(function (r, i) {
        return '<tr><td class="nm"><b class="rk">' + (i + 1) + '</b>' + esc(r.name) +
          ...
          (r.last ? '<span class="lastuse">마지막 발주 ' + esc(r.last) + '</span>' : '') +
          '</td><td class="q">' + num(r.n) + '건</td></tr>';
      }).join('') + '</tbody></table>';
  }
  if (named.length) {
    b += '<h4>기록에 적힌 이름 (P/N 없음)</h4><div class="chips">' +
  ...
  b += '<div class="note">건수는 그 Part 가 이 Error 방문에서 청구된 <b>방문 수</b>이고, ' +
       '<b>마지막 발주</b>는 그 P/N 을 마지막으로 발주한 달입니다. ' +
...
function relatedHTML(e, dev) {
  ...
  return '<details class="acc dat"><summary>Related Errors<span class="src">데이터</span>' +
    ...
    '<div class="note">같이 뜬 것일 뿐, 어느 쪽이 원인인지는 확인되지 않았습니다.</div>' +
    '</div></details>';
}
...
function notesHTML(e) {
  ...
  return '<details class="acc crm"><summary>원인 (현장 기록)' + peek +
    ...
    '숫자로 거꾸로 짚은 추정이 아니라 <b>사람이 적어 둔 원인</b>이며, 정형 문구는 걸렀습니다.</div>' +
    '</div></details>';
}
...
function renderError(dev, part, idx) {
  ...
  loadDevice(dev).then(function (j) {
    ...
    titleEl.className = 'long';   // 에러 이름은 잘리면 뜻을 잃는다
    ...
    var head =
      ...
      '<span class="tag dv" style="--dc:' + devColor(dev) + '">' + esc(dev) + '</span>' +
